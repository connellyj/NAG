...
{ 
    ...
    void Update()
    {
        if (trackedController.triggerPressed) DoClick();
    }
    ...
    void DoClick()
    {
        if (teleportOnClick)
        {
            ...
            if (teleportType == TeleportType.TeleportTypeUseTerrain) // If we picked to use the terrain
            {
                ...
                if (hasGroundTarget) drawLine(transform.position, hitInfo.point);
                else
                {
                    lr.enabled = false;
                    sphere.SetActive(false);
